...
type WebchatFormAction = 'data_exchange' | 'INIT' | 'BACK';

interface BmForm {
  /**
   * Name of the next screen to navigate to. Set to 'SUCCESS' or leave undefined to finish the form.
   */
  nextScreen?: string;
  /**
  ...
  data?: { [key: string]: any };
  /**
   * Finish endpoint execution and resume the form on Webchat. Always call this last.
   */
  send: () => void;
}

interface BmFormConsole {
  log: (...args: any[]) => void;
  warn: (...args: any[]) => void;
  error: (...args: any[]) => void;
}
...
declare global {
   ...
   * - 'INIT': sent when the message that opens the form is marked with 'data_exchange'.
   * - 'BACK': sent when a screen has refresh_on_back === true. You must provide data for the previous screen.
   */
  const action: WebchatFormAction;
  /**
   * Name of the screen the user is leaving. Set when action is 'data_exchange' or 'BACK'.
   */
  const screen: string;
  /**
   * Values sent from the form JSON in "payload", usually the user input.
   */
  const data: { [key: string]: any };
  /**
   * Response object. Set `nextScreen` and/or `data`, then call `send()` to resume the form.
   */
  const form: BmForm;

  const bmconsole: BmFormConsole;
